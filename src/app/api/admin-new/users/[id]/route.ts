...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Build a single query that matches either ObjectId or string _id
    const filters: any[] = [];
    try {
    ...
    console.debug("Updating user with query:", query);

    let result: any = null;
    try {
      result = await usersCollection.findOneAndUpdate(query, { $set: update }, { returnDocument: "after" });
    } catch (e) {
      console.error("findOneAndUpdate failed for query", query, e);
    }

    if (!result || !result.value) {
      console.error("User not found for id", id, "with query", query);
      return NextResponse.json({ error: "User not found" }, { status: 404 });
    }

    return NextResponse.json({ message: "Updated", user: result.value });
  } catch (err) {
    console.error("PATCH admin-new user error:", err);
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    console.error("GET admin-new user error:", err);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
